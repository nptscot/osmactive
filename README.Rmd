...
```{r auckland}
library(sf)
zones = zonebuilder::zb_zone("Auckland")
zones_6km = zones |>
  dplyr::filter(circle_id <= 3) |>
  sf::st_union()
# mapview::mapview(zones_6km)
...
#| include: false
# Save an interactive version of the map to check the resu lts as follows:
tmap_save(m, "cycle_net_auckland.html")
browseURL("cycle_net_auckland.html")
system("gh release upload v0.1 cycle_net_auckland.html --clobber")
# Available:
browseURL("https://github.com/nptscot/osmactive/releases/download/v0.1/cycle_net_auckland.html")
```