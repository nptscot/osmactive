...
```{r leeds}
osm = get_travel_network("Jamaica")
cycle_net = get_cycling_network(osm)
drive_net = get_driving_network(osm)
cycle_net_d = distance_to_road(cycle_net, drive_net)
cycle_net_c = classify_cycle_infrastructure(cycle_net_d)
...
```{r}
#| include: false
tmap_save(m, "cycle_net_jamaica.html")
browseURL("cycle_net_jamaica.html")
system("gh release upload v0.1 cycle_net_jamaica.html --clobber")
# Available:
