get_pavement_widths = function(road_geojson_path, roadside_geojson_path, crs = "EPSG:27700", buffer_dist = 15, segment_length = 20) {
  ...
  if (segment_length > 0) {
      ...
      if (rsgeo_installed) {
          ...
          roads_split = stplanr::line_segment(roads_sf, segment_length = segment_length)
      }
  ...
  roads_geos = geos::as_geos_geometry(roads_sf)
  roadside_polygons_geos = geos::as_geos_geometry(sf::st_geometry(roadside_polygons_sf))

  # Create buffered roads using geos
  roads_buffered = geos::geos_buffer(roads_geos, dist = buffer_dist, params = geos::geos_buffer_params(end_cap_style = "flat"))
  roads_buffered_right = geos::geos_buffer(roads_geos, dist = buffer_dist, params = geos::geos_buffer_params(end_cap_style = "flat", single_sided = TRUE))
  roads_buffered_left = geos::geos_difference(roads_buffered, roads_buffered_right)

  # Convert geos geometries back to sf for attaching average widths and further processing
  average_widths_total = calculate_widths(roads_geos, roadside_polygons_geos, roads_buffered)
  ...
  average_widths_right = calculate_widths(roads_geos, roadside_polygons_geos, roads_buffered_right)
  roads_sf$average_widths_right = average_widths_right

  average_widths_left = calculate_widths(roads_geos, roadside_polygons_geos, roads_buffered_left)
  roads_sf$average_widths_left = average_widths_left

  return(roads_sf)
}

calculate_widths = function(roads_geos, roadside_polygons_geos, roads_buffered) {
  average_widths = numeric(length = length(roads_geos))

  for (i in seq_along(roads_geos)) {
    intersections = geos::geos_intersects(roads_buffered[i], roadside_polygons_geos)
    ...
    if (length(relevant_indices) > 0) {
      ...
      pavements_intersection_within = geos::geos_intersection(relevant_polygons, roads_buffered[i])
