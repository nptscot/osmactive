...
level_of_service = function(osm) {
  osm = osm |>
    dplyr::mutate(`Level of Service` = dplyr::case_when(
      ...
      detailed_segregation == "Level track" & (is.na(final_speed) | is.na(final_traffic)) ~ "Medium",
      detailed_segregation == "Footway" & (is.na(final_speed) | is.na(final_traffic)) ~ "Medium",
