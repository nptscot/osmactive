...
system("gh release upload v0.1 gisruk2025.pdf")
```
...
The CbD guidance specifies the minimum width required for cycle infrastructure, which is summarised in table @tbl-cbd.
...
The CbD guidance specifies buffers that must be accounted for when calculating the effective available width for cycle infrastructure (@tbl-buffers).
...
The resulting data was visualised in the NPT's street space layer, as shown in @fig-street-space.
...
Thanks to Ordnance Survey for providing the MasterMap data that underpins the NPT (contains OS data © Crown copyright and database rights 2025 OS licence number 100046668).
...
Juan Jose Fonseca Zamora is a PhD student at the Leeds Institute for Transport Studies (ITS) and specialises in new methods for traffic modelling.
