...
mapview::mapview(cycle_net_f, color = "blue") + 
  mapview::mapview(drive_net_f, color = "red") 

get_azimuth <- function(line) {
  coords <- sf::st_coordinates(line)
  if (nrow(coords) < 2) return(NA)
  x1 <- coords[1, "X"]
  y1 <- coords[1, "Y"]
  x2 <- coords[nrow(coords), "X"]
  y2 <- coords[nrow(coords), "Y"]
  atan2(x2 - x1, y2 - y1) * 180 / pi
}

find_nearby_speeds <- function(cycle_net, drive_net, buffer_dist = 10, angle_threshold = 20) {
  # Filter for cycle infrastructure without maxspeed
  cycle_infra_without_maxspeed <- cycle_net |>
    filter(is.na(maxspeed))
  
  # Calculate azimuth for cycle infrastructure
  cycle_infra_without_maxspeed$azimuth <- sapply(
    sf::st_geometry(cycle_infra_without_maxspeed), 
    get_azimuth
  )
  
  # Buffer the cycle infrastructure
  cycle_infra_without_maxspeed_buffer <- sf::st_buffer(cycle_infra_without_maxspeed, dist = buffer_dist)
  
  # Filter roads with maxspeed and calculate azimuth
  roads_with_maxspeed <- drive_net |>
    filter(!is.na(maxspeed))
  
  roads_with_maxspeed$azimuth <- sapply(
    sf::st_geometry(roads_with_maxspeed), 
    get_azimuth
  )
  
  # Create points for spatial join
  roads_with_maxspeed_points <- sf::st_point_on_surface(roads_with_maxspeed)
  
  # Join cycle infrastructure with nearby roads that have similar orientation
  joined_data <- sf::st_join(
    cycle_infra_without_maxspeed_buffer,
    roads_with_maxspeed_points,
    join = sf::st_intersects
  ) |>
    mutate(angle_diff = abs((azimuth.x - azimuth.y) %% 180)) |>
    filter(angle_diff <= angle_threshold | (180 - angle_diff) <= angle_threshold) |>
    group_by(osm_id.x) |>
    slice(1) |>
    ungroup()
...
update_missing_speeds <- function(cycle_net, joined_data) {
  # Join the original cycle network with the matched speeds
  cycle_net_updated <- cycle_net |>
    left_join(
      joined_data |>
        sf::st_drop_geometry() |>
        select(osm_id.x, maxspeed.y),
      by = c("osm_id" = "osm_id.x")
    ) |>
    mutate(maxspeed = ifelse(is.na(maxspeed), maxspeed.y, maxspeed)) |>
    select(-maxspeed.y)
  
  return(cycle_net_updated)
}

# Example usage:
joined_data <- find_nearby_speeds(cycle_net_f, drive_net_f)
cycle_net_f_updated <- update_missing_speeds(cycle_net_f, joined_data)
